...
- 不引入组件库 / CSS 框架；性能条目保持既有外观。用户确认的 UI 小批次（OPT-29/30）仅允许局部辅助文字对比度、原生列表按钮语义与键盘焦点轮廓调整，沿用布局、字号、token 和平台兼容路径。
...
### OPT-27 【P1·性能】闲置会话与传输缓存回收

- 状态：已完成（v0.54.8，决议 #298）
- **问题**：真实 Electron Node16 探针打开 100 会话，再灌入两轮各 5 万条后台消息及 5000 条终态传输；原实现驻留消息从 5000 增至 104250 条、终态及测速样本各 10000 条，GC 后 JS 堆从 6.71 增至 44.32MiB。
- **方案**：闲置会话按最近使用顺序整组回收，最多 10 个且合计 3000 条消息；当前会话和移除撤回项豁免。传输按卡片生命周期引用，保留活动传输及被引用状态，未引用终态最多 200 条；终态释放测速样本。
- **兼容**：只释放内存投影与索引，重新进入沿用既有最新页/历史跳转；保护迟到分页与实时传输优先，保留未读、撤回移除、转发、群文件汇总、重试及平台分支。
- **验收**：相同 100 会话压力探针对照，活动/待撤回保护、淘汰后重载与目标跳转、迟到分页、共享卡片引用与传输推送竞态测试；五连验证与 macOS 实窗检查。

---

### OPT-28 【P1·性能】缩略图冷缓存并发与排队需求回收

- 状态：已完成（v0.54.9，决议 #299）
- **问题**：24 张 1600×1200 静态 PNG 冷缓存探针在真实 Electron Chrome108 软件渲染环境同时出现 24 个 bitmap 任务，工作集峰值 416.52MiB；2 路队列原型降为 199.80MiB，首张生成 172.7 → 18.3ms（本机合成样本，最终复测为准）。
- **方案**：流水线软件/未知配置 2 路、硬件 4 路；同 ID 共享需求，离开近视口/改绑/卸载只移除自己的需求，排队项无人需要则丢弃。完成结果复用已有 512 项 LRU，进行中任务持续共享。
- **兼容**：保留原图、动图、小图与失败退路、独立查看/OCR、480px 观察范围和所有像素/磁盘门禁；不引入新依赖或 IPC。
- **验收**：真实 Chrome108 冷缓存新旧性能对照、两种配置并发上限、共享需求/快速进出/改绑/卸载、缓存命中与失败释放测试；五连验证和 macOS 实窗原图/缩略图查看。

---

### OPT-29 【P2·UI】列表辅助文字对比度

- 状态：已完成（v0.54.11，决议 #301）
- **问题**：浅色列表上的 `--text-3` 约 2.96:1，时间、摘要、组织路径和 IP 阅读费力。
- **方案**：会话时间/摘要/计数/空态、通讯录计数/IP/离线昵称/空态、搜索分类/摘要/组织路径/离线说明/状态提示局部使用现有 `--text-2`。保留灰色离线语义、状态点、禁用按钮和全局 token；不调整字号、行高、布局与阴影。
- **验收**：核算浅深色及悬停/置顶/选中背景的辅助文字对比度，实窗对照三类列表、长名称和既有缩放；五连验证。

---

### OPT-30 【P2·UI】列表键盘可达性

- 状态：已完成（v0.54.12，决议 #302）
- **问题**：会话、通讯录分组/联系人及搜索结果的 li/div 仅绑定鼠标操作，无法通过 Tab/Enter/Space 使用。
- **方案**：保留列表结构，行入口使用原生按钮与局部焦点轮廓；继承字体、显式重置按钮默认边框/背景/对齐。沿用原有点击、双击及右键回调。分组展开、当前会话与联系人状态提供可访问语义。
- **兼容**：不引入键盘库、全局监听或新快捷键；不改 IME、Esc、平台分支及默认焦点策略，保留减少动态效果。
- **验收**：实窗 Tab/Shift+Tab、Enter/Space、分组折叠和资料/开聊/搜索三类跳转、鼠标单双击/右键及 Esc；浅深色、缩放、省略和选中焦点检查；五连验证。
...
| K（2026-09 评审第二批） | OPT-26 → OPT-27 → OPT-28 | 用户已确认执行；先测量，再独立提交 |
| L（2026-09 UI 小批次） | OPT-29 → OPT-30 | 用户已确认执行；局部文字对比度与列表键盘可达性，逐项验证提交 |
...
- 2026-09-05 v1.21 决议 #298：OPT-27 根据缓存持续增长探针增加闲置回收，计划版本 **0.54.8**。
- 2026-09-05 v1.22 OPT-27 完成：680 项测试与五连验证全过。相同 100 会话、两轮 5 万消息/5000 终态探针，消息驻留 **104250 → 2400**，传输 **10000 → 200**、终态测速样本 **10000 → 0**，GC 后 JS 堆 **44.32 → 7.76MiB**（仅此合成场景）。当前 5000 条历史、撤回窗口、转发引用、未读及迟到分页/推送测试通过；macOS 实窗依次打开 15 个会话/300 条文件状态，再回首个被淘汰会话，20 张文件卡全部正确恢复。App JS 797589 字节，四窗口预算通过。
- 2026-09-05 v1.23 决议 #299：OPT-28 根据冷缓存 bitmap 和工作集探针约束缩略图流水线并发，计划版本 **0.54.9**。
- 2026-09-05 v1.24 OPT-28 完成：694 项测试、真实 Electron 数据库自测、类型检查、构建和回环 smoke 全过。Chrome108 / macOS arm64、24 张 1600×1200 PNG、每种配置新旧实现交替各跑 5 次取中位数：软件渲染 bitmap 峰值 **24 → 2**，工作集峰值 **411.83 → 207.06MiB**，首张生成 **132.3 → 19.3ms**，全部完成 **137.1 → 171.0ms**；硬件渲染 bitmap **24 → 4**，工作集 **409.33 → 220.06MiB**，首张 **158.3 → 35.2ms**，全部 **161.0 → 153.5ms**。软件渲染以约 34ms 批量收尾时间换取较低峰值和更早首张，测量限于本机合成场景。macOS 实窗冷缓存、滚动/切换、动图/小图及独立原图 100% 查看通过；生成 7 张 320×240 派生图，动图与小图均未生成缩略图，1600×1200 原图保持。公共启动闭包 80288 字节、App JS 798737 / CSS 115605 字节，四窗口预算通过。第二批 OPT-26–28 完成；Win7/UOS 等真实目标平台测试保留既有外部验证边界。
- 2026-09-05 v1.25 决议 #301：启动用户确认的 UI 小批次，OPT-29 仅调整三类列表的辅助文字 token，计划版本 **0.54.11**。
- 2026-09-05 v1.26 OPT-29 完成：695 项测试、Electron Node16 数据库自测、类型检查、构建和回环 smoke 全过。按 `material-panel` 叠加 `bg-chat` 后核算普通/置顶/悬停/选中/搜索悬停背景，本次辅助文字浅色 **4.74–5.60:1**、深色 **6.23–8.79:1**；macOS 实窗浅色 100% 和深色 125% 的会话、通讯录、搜索长文本及省略显示通过。仅改 12 处局部文字色，App JS/CSS 字节数不变，四窗口预算通过。
- 2026-09-05 v1.27 决议 #302：OPT-30 以原生按钮补列表键盘操作，复用原有回调与字体布局，计划版本 **0.54.12**。
- 2026-09-05 v1.28 OPT-30 验证进度：695 项测试、Electron Node16 数据库自测、类型检查、构建和回环 smoke 全过。macOS / Chrome108 深色 125% 实窗通过 Tab/Shift+Tab、Enter/Space 切换会话、分组折叠/展开、资料页键盘开聊、鼠标双击、右键菜单及 Esc、联系人/聊天/文件三类搜索跳转；已修正原生按钮默认对齐引起的长摘要溢出。最后补文件名省略容器后，浅色 100% 焦点和文件名复测因 Mac 锁屏未执行，解锁后继续。App JS 799153 / CSS 116162 字节，公共启动 80288 字节，四窗口预算通过；Win7/UOS 实机仍按既有外部验证边界执行。
- 2026-09-06 v1.29 OPT-30 完成：Mac 解锁后补齐最终浅色 100% 实窗复核，会话选中/键盘焦点共存、通讯录资料页入口、长文件名省略、空格定位文件消息均通过。结合既有深色 125% 与五连验证，UI 小批次 OPT-29/30 验收完成，发布版本 **0.54.12**。
