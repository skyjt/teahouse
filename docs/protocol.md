...
| | |
|---|---|
| 状态 | v0.16，P1 协议基线已落地；心跳/重试等时延参数实现期按实测微调 |
| 日期 | 2026-06-12 |
...
- 群元数据：`{groupId, name, members[nodeId…], rev, updatedBy, updatedTs, creatorIp, adminSecretHash}`，**rev 单调递增，冲突按 (rev, updatedTs) 取大者**（LWW，尽力而为一致性，需求 F-MSG-4）。
...
- 上限 50 人/组；群管理指改名、添加成员、移出他人。建群时记录 `creatorIp`，并可选管理密码：有密码的组同步 `adminSecretHash`（密码明文不入库、不入协议），成员输入密码后可发起管理变更；无密码的组仅允许 `creatorIp` 对应来源 IP 的管理变更。成员自行退组不需要管理权限。
