...
| OPT-28 | P1 | Bound thumbnail pipelines and drop unused queued demand | Complete (#299), v0.54.9 |
| OPT-29 | P2 | Improve list metadata contrast with the existing secondary token | Complete (#301), v0.54.11 |
| OPT-30 | P2 | Native keyboard actions and focus indicators for lists | Complete (#302), v0.54.12 |
...
- **2026-09-05, decision #299:** OPT-28 uses 2 software/unknown-profile or 4 hardware thumbnail slots, shared same-ID demand, viewport/unmount/rebind cancellation of undemanded queued work, and the existing completed-result LRU. A real Chrome108 software-rendered probe with 24 cold 1600×1200 PNGs measured 24 simultaneous bitmaps and 416.52MiB peak working set; a 2-slot prototype measured 199.80MiB and first generation 172.7 → 18.3ms. Final implementation will be remeasured. Planned version **0.54.9**.
- **2026-09-05, OPT-28 complete:** 694 tests and all five local checks passed. Real Chrome108/macOS arm64, 24 cold 1600×1200 PNGs, five alternating runs per implementation/configuration: software bitmaps **24 → 2**, peak working set **411.83 → 207.06MiB**, first generation **132.3 → 19.3ms**, full batch **137.1 → 171.0ms**; hardware bitmaps **24 → 4**, peak working set **409.33 → 220.06MiB**, first **158.3 → 35.2ms**, full batch **161.0 → 153.5ms**. The software case trades about 34ms of batch completion time for lower peak memory and an earlier first image; these are local synthetic measurements. Native macOS cold loading, scrolling/switching, animation/small-image fallbacks, and 100% original viewing passed. Seven 320×240 derivatives were generated, none for animation/small images; the 1600×1200 source stays intact. Common startup is 80288 bytes; App JS/CSS are 798737/115605 bytes; all four window budgets passed. Second batch OPT-26–28 is complete; real Windows 7/UOS verification remains target-platform work.
- **2026-09-05, decision #301:** start the user-approved UI batch. OPT-29 applies the existing secondary token to readable list metadata locally; preserve global tokens, layout, disabled/offline semantics and platform behavior. Planned version **0.54.11**.
- **2026-09-05, OPT-29 complete:** 695 tests and all five checks passed with loopback smoke. Composite list backgrounds yield **4.74–5.60:1** light and **6.23–8.79:1** dark for the adjusted metadata. Native macOS checks cover all three lists, truncation, light 100% and dark 125%. Twelve local text-color substitutions; App JS/CSS sizes are unchanged and all four budgets passed.
- **2026-09-05, decision #302:** OPT-30 uses native buttons for keyboard list actions and local focus indicators, preserving existing callbacks and platform behavior. Planned version **0.54.12**.
- **2026-09-05, OPT-30 verification:** 695 tests and all five checks passed with loopback smoke. Native Chrome108/macOS dark 125% passed Tab/Shift+Tab, Enter/Space conversation switching, contact-group toggling, profile-to-chat keyboard navigation, double-click chat, context-menu Escape and all three search categories. Fixed native-button alignment that caused long-summary overflow. After the final filename truncation span, the light 100% focus/filename recheck was blocked by the locked Mac and remains pending. App JS/CSS 799153/116162 bytes; common startup 80288 bytes; all four budgets passed. Windows 7/UOS hands-on validation remains target-platform work.
- **2026-09-06, OPT-30 complete:** after unlocking the Mac, the final light 100% native-window checks passed: selected conversation with keyboard focus, contact profile navigation, long-filename ellipsis and Space navigation to the file message. Together with dark 125% and the five local checks, UI batch OPT-29/30 is complete for release **0.54.12**.
